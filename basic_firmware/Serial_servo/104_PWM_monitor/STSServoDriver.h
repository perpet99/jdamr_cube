...
public:
    ...
    /// \brief Get current servo torque.
    /// \param[in] servoId ID of the servo
    /// \return Current torque, in A.
    bool setTorque(byte const &servoId, bool const &on);

    signed char getCurrentPWM(byte const &servoId);
    ...
    // only for getCurrentPWM()
    signed char readRegisterEx(byte const &servoId, byte const &registerId);
