...
signed char STSServoDriver::getCurrentPWM(byte const &servoId)
{
    byte rawValue = readRegister(servoId, STSRegisters::CURRENT_DRIVE_VOLTAGE);
    return static_cast<signed char>(rawValue);
}
...
bool STSServoDriver::setTorque(byte const &servoId, bool const &on)
{
    return writeRegister(servoId, STSRegisters::TORQUE_SWITCH, on ? 1 : 0);
}

// only for getCurrentPMW() functions
